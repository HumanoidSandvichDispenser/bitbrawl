...
namespace Nez.Sprites
{
	...
	public class SpriteRenderer : RenderableComponent
	{
		...
		/// <summary>
		/// Offset vector to apply when rendering the sprite.
		/// </summary>
		public Vector2 RenderOffset { get; set; } = Vector2.Zero;

		/// <summary>
		/// Offset rotation to apply when rendering the sprite.
		/// </summary>
		public float RenderRotationOffset { get; set; } = 0f;
		...
		{
			batcher.Draw(Sprite, Entity.Transform.Position + LocalOffset + RenderOffset, Color,
				Entity.Transform.Rotation + RenderRotationOffset, Origin, Entity.Transform.Scale, SpriteEffects, _layerDepth);
		}
	}
}